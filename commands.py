...
def run_command_list_keys(pkcs11, wallet_id=0, pin=None):
    ...
    try:
        ...
        if session_opened and not had_error:
            ...
            def collect_named_attributes(handle, attr_specs):
                attr_types = [attr_type for attr_type, _ in attr_specs]
                attr_map = safe_get_attributes(pkcs11, session, handle, attr_types)
                if attr_map is None:
                    return None

                result = {}
                for attr_type, attr_name in attr_specs:
                    if attr_type in attr_map and attr_map[attr_type] is not None:
                        result[attr_name] = attr_map[attr_type]

                return result

            def collect_public_attributes(handle):
                attrs = collect_named_attributes(
                    handle,
                    [
                        (CKA_LABEL, 'CKA_LABEL'),
                        (CKA_ID, 'CKA_ID'),
                ...
                if key_type == CKK_GOSTR3410:
                    extra_specs = [(CKA_VALUE, 'CKA_VALUE')]
                ...
                if extra_specs:
                    extra_attrs = collect_named_attributes(handle, extra_specs)
                    if extra_attrs:
                        attrs.update(extra_attrs)

                return attrs

            def collect_private_attributes(handle):
                return collect_named_attributes(
                    handle,
                    [
                        (CKA_LABEL, 'CKA_LABEL'),
                        (CKA_ID, 'CKA_ID'),
            ...
            for handle in search_objects(CKO_PUBLIC_KEY):
                attrs = collect_public_attributes(handle)
            ...
            if logged_in:
                for handle in search_objects(CKO_PRIVATE_KEY):
                    attrs = collect_private_attributes(handle)
            ...
            for idx, pair in enumerate(sorted_pairs, start=1):
                ...
                if (
                    ...
                    and private_entry
                    and private_entry[1]
                    and 'CKA_KEY_TYPE' in private_entry[1]
                ):
                ...
                print(f'  Ключ №{idx} (key-number={idx}){suffix}:')
                ...
                def print_attribute(name, raw_value, hex_only=False):
                    ...
                    emit('HEX', 'hex', truncate=name not in no_truncate_attrs)
                    if not hex_only:
                ...
                if private_entry:
                    _, attrs = private_entry
                    print('    Закрытый ключ')
                    if attrs is not None:
                        for name in ['CKA_LABEL', 'CKA_ID', 'CKA_VALUE']:
                            raw = attrs.get(name) if attrs else None
                            if (
                                raw is None
                                and name == 'CKA_LABEL'
                                and public_entry
